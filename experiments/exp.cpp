...
void genblockedcells()
{
    ...
    numfcells = 0;
    for (i = 0; i < N; i++) for (j = 0; j < M; j++) if (B[i][j] == true) numfcells++;
}
...
//	else if (top == 1) gluLookAt(0,0,0,	0,0,-1,	0,1,0);
	...
	****************************/
    ...
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < M; j++)
        {
            glColor3f(0, 1 - (double)(G[i][j] / (double)lmax), 0);
            glBegin(GL_QUADS);
            {
...
void animate(int val){
    ...
    if (T >= lmax && T < lmax + epoch)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < M; j++)
            {
                if (B[i][j] == true) sumL += G[i][j];
            }
        }
        //cout << (double)sumL / (double)numfcells << endl;
        metric += ((double)sumL / (double)numfcells);
...
void init(){
    ...
    cout << "Grid cells processed." << endl << endl;
    ...
    cout << endl << metric / (epoch * lmax) << endl;
